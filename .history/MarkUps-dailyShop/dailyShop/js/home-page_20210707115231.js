...
function check_login_logout(){
    if(localStorage.getItem("userdata")!==null){
        ...
        for (let i = 0; i < headeradmin.length; i++) {
            ...
            "<li class=\"hidden-xs\"><a href=\"javascript:Logout()\">Quản lý user</a></li>";
            "<li class=\"hidden-xs\"><a href=\"javascript:Logout()\">Logout</a></li>";
          };
    }
};
...
function getAllUsers(){
    $.ajax({
        ...
        success: function (data){
            console.log(data);
            let content = "<tr><th>tên tài khoản</th><th>địa chỉ</th><th>số điện thoại</th><th>trạng thái</th></tr>";
            let tableusers = document.getElementById("table-user-manager");
            for (let i = 0; i < data.length; i++) {
               content += "<tr>"+data.name+"</tr>"+"<tr>"+data.address+"</tr>"+"<tr>"+data.phoneNumber+"</tr"+"<tr>"+data.status+"</tr"
              };
