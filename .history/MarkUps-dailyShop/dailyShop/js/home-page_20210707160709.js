...
function renderAllUser(data){
    let content = "<tr><th>tên tài khoản</th><th>địa chỉ</th><th>số điện thoại</th><th>trạng thái</th><th>Shop name</th><th>Khoá tài khoản</th></tr>";
    let tableusers = document.getElementById("table-user-manager");
    for (let i = 0; i < data.length; i++) {
       content += "<tr><td>"+data[i].name+"</td>"+"<td>"+data[i].address+"</td>"+"<td>"+data[i].phoneNumber+"</td>"+"<td>"+
       data[i].status+"</td>"+"<td>"+data[i].shopName+"</td><td><button type=\"button\" onclick=\"changeStatusUser("+data[i].userName+")class=\"btn btn-warning\">"+data[i].status+"</button>
       <a href=\"javascript:changeStatusUser("+data[i].userName+")\">"+data[i].status+"</a></td></tr>"
      };
